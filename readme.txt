=== WordPress Social Ring (Facebook Like, Google +1, ReTweet, LinkedIn and Pin It) ===
Contributors: niccolo_tapparo
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=niccolo%2etapparo%40gmail%2ecom&lc=IT&item_name=Niccolo%20Tapparo&item_number=1&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donate_SM%2egif%3aNonHosted
Tags: Google +1, Facebook, Facebook Like, Facebook Share, Open Graph Meta Tag, Twitter, Retweet, Google+, Social Network, Linkedin
Requires at least: 3.2
Tested up to: 3.4
Version: 1.1.8
Stable tag: 1.1.8

WordPress Social Ring adds sharing buttons for Facebook, Twitter and Google+.

== Description ==

Let visitors share posts/pages on Facebook, Twitter, Google+, LinkedIn and Pinterest.

From admin page you can choose which button display: Facebook Like, Facebook Send, Facebook Share, Google +1, Twitter, Pin It, in Share. 

You can also choose on which pages display them: Home, Posts, Pages, Categories and Archives.

Follow me for the updates!
http://www.facebook.com/pages/DrWordPress/166397626712895
http://wordpress.altervista.org/en/tag/wordpress-social-ring-en/

== Installation ==

1. Upload the `wp-social-ring` folder to the `/wp-content/plugins/` directory
1. Activate WordPress Social Ring plugin through the 'Plugins' menu in WordPress
1. Configure the plugin by going to the Social Ring menu that appears in your admin menu


== Screenshots ==

1. The WordPress Social Ring option Page.
2. Social sharing buttons on single post.

== Changelog ==

= 1.1.8 =

* Bugfix

= 1.1.7 =

* Added LinkedIn button

= 1.1.6 =

* Bugfix
* Spanish traslation


= 1.1.5 =

* CSS Fix

= 1.1.4 =

* Added Pin It button
* Added button preview on admin page

= 1.1.3 =

* widget bugfix

= 1.1.2 =

* Added shortcode [socialring]
* IE8 fix

= 1.1.1 =

* Widget compatibility fix

= 1.1 =

* Added template tag
* Added Social Ring Widget

= 1.0.6 =

* fixed bug with category and archives

= 1.0.5 =

* compatibility fix

= 1.0.4 =
 
* changed social-ring id to class


= 1.0.3 =
 
* Fixed Duplicated Id for twitter button

= 1.0.2 =

* Fixed bug with WPLANG for Facebook API (thanks to zabor)
* Disabled for custom post types

= 1.0.1 =

* Fixed bug with og:descr (esc_attr)

= 1.0 =

* Fixed Bug with Open Graph meta tag description
* Added Facebook API Localization (it takes WPLANG)
* Added Old Sharing Button

= Beta =

* First Release
